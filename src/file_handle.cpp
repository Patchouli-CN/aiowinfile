...
PyObject *FileHandle::read(int64_t size) {
...
PyObject *FileHandle::write(Py_buffer *view) {
...
PyObject *FileHandle::seek(int64_t offset, int whence) {
    PyObject *future = PyObject_CallNoArgs(m_create_future);
    if (!future) return nullptr;
    {
        std::lock_guard<std::mutex> lk(m_posMtx);
...
void FileHandle::complete_ok(IORequest *req, DWORD bytes) {
...
void FileHandle::complete_error(IORequest *req, DWORD err) {
    m_pending.fetch_sub(1, std::memory_order_release);
    PyGILState_STATE gs = PyGILState_Ensure();
...
IORequest *FileHandle::make_req(size_t size, PyObject *future, ReqType type) {
...
void FileHandle::complete_error_inline(IORequest *req, DWORD err) {
    m_pending.fetch_sub(1, std::memory_order_relaxed);
    PyObject *exc_class = map_win_error(err);
    PyObject *exc = PyObject_CallFunction(exc_class, "is", (int)err, "I/O operation failed");
    PyObject *set_fn = req->set_exception; req->set_exception = nullptr;
...
void FileHandle::resolve_ok(PyObject *future, PyObject *val) {
    PyObject *fn = PyObject_GetAttr(future, g_str_set_result);
    PyObject *r  = PyObject_CallOneArg(fn, val);
    Py_XDECREF(r); Py_DECREF(fn);
}

void FileHandle::resolve_bytes(PyObject *future, const char *buf, Py_ssize_t n) {
    PyObject *b = PyBytes_FromStringAndSize(buf, n);
    resolve_ok(future, b); Py_DECREF(b);
}

void FileHandle::resolve_exc(PyObject *future, PyObject *cls, DWORD err, const char *msg) {
    PyObject *exc = err
